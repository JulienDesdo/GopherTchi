...
static void sizeForMenuBar(NSImage *image) {
  NSSize pixelSize = [image size];
  const CGFloat maxSide = 22.0;
  if (pixelSize.width <= 0 || pixelSize.height <= 0) {
    [image setSize:NSMakeSize(maxSide, maxSide)];
    return;
  }
  CGFloat scale = MIN(maxSide / pixelSize.width, maxSide / pixelSize.height);
  [image setSize:NSMakeSize(pixelSize.width * scale, pixelSize.height * scale)];
}

void setIcon(const char* iconBytes, int length, bool template) {
  NSData* buffer = [NSData dataWithBytes: iconBytes length:length];
  NSImage *image = [[NSImage alloc] initWithData:buffer];
  sizeForMenuBar(image);
